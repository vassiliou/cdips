...
class FCN16VGG:
    ...
    def build(self, rgb, train=False, num_classes=20, random_init_fc8=False,
        ...
        if random_init_fc8:
            ...
            self.score_fr = self._pruned_fc(self.fc6,FC8_SHAPE,[1,1,4096,4096],'fc8',relu=False)
...
###### the pruned layers
    ...
    def _prune_output(self, fweight, shape, num_new):
        print('Pruning output')
        print(shape)
        print(num_new)
        print(fweight.shape)
        num_orig = fweight.shape[1]
        shape[3] = num_new
        fweight=fweight.reshape([1,1,fweight.shape[0],fweight.shape[1]])
        if num_orig <= num_new:
            return fweight
        #assert(num_new < num_orig)
        n_averaged_elements = num_orig//num_new
    ...
    def _prune_input(self, fweight, shape, num_new):
        print('Pruning input')
        print(shape)
        print(num_new)
        print(fweight.shape)
        num_orig = fweight.shape[0]
        shape[2] = num_new
        #assert(num_new < num_orig)
        n_averaged_elements = num_orig//num_new
        fweight=fweight.reshape([1,1,fweight.shape[0],fweight.shape[1]])
        ...
        for i in range(0, num_orig, n_averaged_elements):
            ...
            if avg_idx == num_new:
                break
            avg_fweight[:, :, avg_idx, :] = np.mean(
                fweight[:, :, start_idx:end_idx, :], axis=2)
        return avg_fweight

    def _pruned_fc(self,bottom,new_size,old_size,name,relu=True):
        """Input new size is length 2 list of dimensions for the new fc7 """
        print('Old size: ' + str(old_size))
        print('New size: ' + str(new_size))

        original_weights = self.data_dict[name][0]
        if new_size[0]<old_size[2]:
            in_pruned = self._prune_input(original_weights, [1,1,new_size[0],old_size[0]],new_size[0])
        else: in_pruned = original_weights
        double_pruned = self._prune_output(in_pruned,[1,1,new_size[0],new_size[1]],new_size[1])
        init = tf.constant_initializer(value=double_pruned,
                                       dtype=tf.float32)
        print('Initializer of' + name + 'shape: ' + str(double_pruned.shape))
