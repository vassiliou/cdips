...
# =============================================
# Presumably all of this is available in settings or model file
FLAGS = tf.app.flags.FLAGS
DOWNSAMPLE_FACTOR = 2
FC6_SHAPE=[512,512]
FC7_SHAPE=[512,512]
FC8_SHAPE=[512,250]
RECORD_DIR = '/home/ubuntu/train_records/'
NUM_CLASSES = 2
NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN = 1000
NUM_EXAMPLES_PER_EPOCH_FOR_EVAL = 500

# ?? CHECK NAME OF THESE
HEIGHT = int(420/DOWNSAMPLE_FACTOR)
WIDTH = int(580/DOWNSAMPLE_FACTOR)
...
# Necessary ?
PATTERN = '*.rec'
def filename(row, model):
    """return the path/filename from a given row in a dataframe"""
    # Set to constants defined in settings
    PATH_TO_TEST = r'../test/'
    PATH_TO_TRAIN = r'../train'

    path = {'test':os.path.join(PATH_TO_TEST, r'/test/{model}/{filename}'),
            'train':os.path.join(PATH_TO_TRAIN, r'/train/{model}/{filename}'}

    # Add 'filename' column to training.bin
    return path[row[model]].format(model=model, filename=row['filename'])
...
def inputs(dataframe, batch_size,train=True):
  ...
  #pattern = os.path.join(data_dir, '*.rec')
  filenames = [filename(row) for row in dataframe.iterrows()]
